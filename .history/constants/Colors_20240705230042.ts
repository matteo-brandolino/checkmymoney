export const THEME = {
  light: {
    background: "hsl(293 33.15% 3.76%)", // background
    border: "hsl(293 25.5% 14.1%)", // border
    card: "hsl(293 33.15% 3.76%)", // card
    notification: "hsl(0 62.8% 30.6%)", // destructive
    primary: "hsl(293 51% 47%)", // primary
    secondary: "hsl(293 25.5% 14.1%)",
    text: "hsl(293 5.1% 97.35%)", // foreground
    gradient:
      "    --gradient: url(/samples/vibrant-vista/vibrant-vista-001.jpg);",
  },
  dark: {
    background: "hsl(222.2 84% 4.9%)", // background
    border: "hsl(217.2 32.6% 17.5%)", // border
    card: "hsl(222.2 84% 4.9%)", // card
    notification: "hsl(0 62.8% 30.6%)", // destructive
    primary: "hsl(217.2 91.2% 59.8%)", // primary
    secondary: "hsl(217.2 32.6% 17.5%)",
    text: "hsl(210 40% 98%)", // foreground,
    gradient: "linear-gradient(to top left, #833ab4, #fd1d1d, #fcb045)",
  },
};
