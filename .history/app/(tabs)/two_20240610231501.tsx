...
function isEntry(arg: Data[] | Entry[] | null | undefined): arg is Entry[] {
  if (!arg) return false;
  console.log(
    "here",
    arg.filter((element) => "key" in element).length === arg.length
  );

  return (
    arg &&
    arg.filter((element) => !(element as Entry).key).length === arg.length
  );
}
